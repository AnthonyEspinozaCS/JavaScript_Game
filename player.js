    ...
    checkCollision(deltaTime){
        this.game.enemies.forEach(enemy => {
            if (enemy.x < this.x + this.width && 
                enemy.x + enemy.width/3 > this.x && 
                enemy.y < this.y + this.height && 
                enemy.y + enemy.height/3 > this.y){
                //collsiion detected
                enemy.markedForDeletion = true;
                this.game.collisions.push(new CollisionAnimation(this.game, enemy.x + enemy.width/3 * 0.5, enemy.y + enemy.height/3 * 0.5));
                if(this.currentState === this.states[4] || this.currentState === this.states[5]){
                    ...
                    if(this.game.time - 1000 >= 0) this.game.time -= 1000;
